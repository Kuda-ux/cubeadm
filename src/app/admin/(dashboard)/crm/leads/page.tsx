...
import { UserPlus, Plus, Search, Mail, Phone, Building2, Loader2, Trash2 } from 'lucide-react'
...
export default function LeadsPage() {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      if (response.ok) setLeads(leads.filter(l => l.id !== id))
    } catch (error) {
      ...
      <div className="bg-white/5 border border-white/5 rounded-2xl overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-12">
            <UserPlus className="w-12 h-12 mx-auto text-gray-500 mb-4" />
            <p className="text-gray-500">No leads found</p>
            <Link href="/admin/crm/leads/new" className="text-[#00D4FF] hover:underline mt-2 inline-block">
              Add your first lead
            </Link>
          </div>
        ) : (
            ...
            <tbody className="divide-y divide-white/5">
              {filteredLeads.map((lead) => (
                <tr key={lead.id} className="hover:bg-white/5">
                  ...
                  <td className="p-4 text-gray-400">{lead.company || 'N/A'}</td>
                  <td className="p-4 text-gray-400">{lead.interest || 'N/A'}</td>
                  <td className="p-4 text-gray-400">{lead.source || 'N/A'}</td>
                  <td className="p-4">
                  ...
                  <td className="p-4">
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => handleDelete(lead.id)} className="text-sm text-red-500 hover:underline">Delete</button>
                    </div>
                  </td>
                </tr>
              ))}
